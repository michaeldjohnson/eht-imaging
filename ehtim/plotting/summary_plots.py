...
################################################################################
        ...
        ha='left',va='center',transform=ax.transAxes)
        ax.text(.23,.3,"%0.1f $\mu$as" % (im_display.fovx()/RADPERUAS), fontsize=fs,
        ha='left',va='center',transform=ax.transAxes)
...
def imgsum_pol(im, obs, obs_uncal, outname,
    ...
    with PdfPages(outname) as pdf:
        ...
        ax=_display_img_pol(imblur, axis=ax, show=False, has_title=False, cfun=plt.get_cmap('bwr'), 
                           pol='Q', polticks=False, 
                           nvec=nvec, pcut=0.1,  mcut=0.01, contour=True,
                           fontsize=fontsize)
        ...
        ax = plt.subplot(gs[0:2,0:2])
        ax.set_title('m')
        ax=_display_img_pol(im, axis=ax, show=True, has_title=False, cfun=plt.get_cmap('jet'), 
        ...
        ax = plt.subplot(gs[2:4,0:2])
        ax.set_title('chi')
        ax=_display_img_pol(im, axis=ax, show=False, has_title=False, cfun=plt.get_cmap('jet'), 
                           pol='chi', polticks=False,
                           nvec=nvec, pcut=0.1,  mcut=0.01, contour=False,
                           fontsize=fontsize)

        ax = plt.subplot(gs[0:2,2:5])
        ax=_display_img_pol(imblur, axis=ax, show=False, has_title=False, cfun=plt.get_cmap('jet'), 
                           pol='m', polticks=False,
                           nvec=nvec, pcut=0.1,  mcut=0.01, contour=False,
                           fontsize=fontsize)
        ...
        for ii in range(0, len(uniquebl)):
            ...
            if len(m_bl)>0:
                ...
                chisq_pvis_bl = np.sum(np.abs((pvis_bl['pvis'] - pvis_bl_model['pvis'])/pvis_bl['psigma'])**2)
                npts_pvis = len(pvis_bl_model)
                data_pvis =  (bl[0],bl[1],npts_pvis,chisq_pvis_bl)
                bl_chisq_data_pvis.append(data_pvis)

                chisq_qvis_bl = np.sum(np.abs((qvis_bl['qvis'] - qvis_bl_model['qvis'])/qvis_bl['qsigma'])**2)
                npts_qvis = len(qvis_bl_model)
                data_qvis =  (bl[0],bl[1],npts_qvis,chisq_qvis_bl)
                bl_chisq_data_qvis.append(data_qvis)

                chisq_uvis_bl = np.sum(np.abs((uvis_bl['uvis'] - uvis_bl_model['uvis'])/uvis_bl['usigma'])**2)
        ...
        chisqtab_m=r"\begin{tabular}{ l|l|l|l } \hline Baseline & $N_{m}$ & $\chi^2_{m}/N_{m}$ & $\chi^2_{m}/N_{total}$ \\ \hline \hline"
        chisqtab_p=r"\begin{tabular}{ l|l|l|l } \hline Baseline & $N_{p}$ & $\chi^2_{p}/N_{p}$ & $\chi^2_{p}/N_{total}$ \\ \hline \hline"
        chisqtab_q=r"\begin{tabular}{ l|l|l|l } \hline Baseline & $N_{Q}$ & $\chi^2_{Q}/N_{Q}$ & $\chi^2_{Q}/N_{total}$ \\ \hline \hline"
        chisqtab_u=r"\begin{tabular}{ l|l|l|l } \hline Baseline & $N_{U}$ & $\chi^2_{U}/N_{U}$ & $\chi^2_{U}/N_{total}$ \\ \hline \hline"
        ...
        if mplots:
            ...
            fig = plt.figure(3, figsize=(18, 28), dpi=200)
            plt.suptitle("Fractional  Polarization Plots",y=.9,va='center',fontsize=int(1.2*fontsize))
            gs = gridspec.GridSpec(6,4, wspace=WSPACE, hspace=HSPACE)
            i = 0
            j = 0
            ...
            for nbl, bl in enumerate(uniquebl):
                ...
                ax = plot_bl_obs_compare(obs_all, bl[0], bl[1], 'mamp', rangey=[0,amax], 
                                         markersize=MARKERSIZE,debias=False, snrcut=snrcut_dict['m'],
                                         axis=ax,legend=False, clist=['k',SCOLORS[1]],
        ...
        if pplots:
            ...
            for nbl, bl in enumerate(uniquebl):
                ...
                ax = plot_bl_obs_compare(obs_all, bl[0], bl[1], 'pamp', rangey=[0,amax], 
                                         markersize=MARKERSIZE,debias=False, snrcut=snrcut_dict['pvis'],
                                         axis=ax,legend=False, clist=['k',SCOLORS[1]],
                                         ttype='nfft',show=False, ebar=ebar)
                ax.set_xlabel('')
                j = 1
        ...
        if qplots:
            ...
            for nbl, bl in enumerate(uniquebl):
                ...
                ax = plot_bl_obs_compare(obs_all, bl[0], bl[1], 'qamp', rangey=[0,amax], 
                                         markersize=MARKERSIZE,debias=False, snrcut=snrcut_dict['qvis'],
                                         axis=ax,legend=False, clist=['k',SCOLORS[1]],
                                         ttype='nfft',show=False, ebar=ebar)
                ax.set_xlabel('')
                j = 1
        ...
        if uplots:
            ...
            for nbl, bl in enumerate(uniquebl):
                ...
                ax = plot_bl_obs_compare(obs_all, bl[0], bl[1], 'uamp', rangey=[0,amax], 
                                         markersize=MARKERSIZE,debias=False, snrcut=snrcut_dict['uvis'],
                                         axis=ax,legend=False, clist=['k',SCOLORS[1]],
                                         ttype='nfft',show=False, ebar=ebar)
                ax.set_xlabel('')
                j = 1
...
def _display_img_pol(im, beamparams=None, scale='linear',gamma=0.5,cbar_lims=False,
                    ...
                    nvec=False, pcut=0.1,  mcut=0.01, contour=False,
                    axis=False,show=False,fontsize=FONTSIZE):
    ...
    #factor = 1.e3 * (1./fovfactor)**2 / (1./im.xdim)**2 
    #unit = 'mJy/$\mu$ as$^2$'
    ...
    if cbar_lims:
        ax = ax.imshow(imarr, cmap=plt.get_cmap(cfun), interpolation=interp, vmin=cbar_lims[0], vmax=cbar_lims[1])  
    else:
        ax = ax.imshow(imarr, cmap=plt.get_cmap(cfun), interpolation=interp)
    ...
    if polticks:
        ...
        if len(ivec)==0: ivec = np.zeros(im_stokes.ydim*im_stokes.xdim)
